# Building and Deploying Logging.Client
...
```bash
git clone https://github.com/openmindednewby/Logging.Client.git
cd Logging.Client
```
...
dotnet add package Logging.Client --version 1.0.1 --source LocalFeed
```
...
```xml
<ItemGroup>
  <PackageReference Include="Logging.Client" Version="1.0.1">
    <Source>C:\path\to\Logging.Client\artifacts\Logging.Client.1.0.1.nupkg</Source>
  </PackageReference>
</ItemGroup>
```
...
2. Click **Create**
3. Provide a key name (e.g., "Logging.Client")
4. Select **Push** scope
5. Select packages or glob pattern (e.g., `Logging.Client*`)
6. Copy the generated API key

### Step 2: Publish

```bash
dotnet nuget push ./artifacts/Logging.Client.1.0.1.nupkg \
...
https://www.nuget.org/packages/Logging.Client
...
unzip Logging.Client.1.0.1.nupkg -d extracted/
...
```
error: Response status code does not indicate success: 409 (Conflict - The feed already contains 'Logging.Client' version '1.0.1'.)
```
...
- NuGet: [NuGet Package Page](https://www.nuget.org/packages/Logging.Client)
